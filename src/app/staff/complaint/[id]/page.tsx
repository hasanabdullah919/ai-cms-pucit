...
export default function StaffComplaintDetail() {
  ...
  const daysOverdue = isOverdue ? differenceInDays(new Date(), new Date(complaint.sla_deadline)) : 0
  const daysSinceCreated = differenceInDays(new Date(), new Date(complaint.created_at))
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-blue-200 text-sm mt-1">
                      Received {format(new Date(complaint.created_at), 'MMM d, yyyy')}
              ...
              <div className="p-6">
                {/* Escalation warning */}
                {daysSinceCreated > 7 && !['resolved','closed'].includes(complaint.status) && (
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4">
                    <div className="flex gap-3">
                      <AlertTriangle className="w-5 h-5 text-red-600 shrink-0" />
                      <div>
                        <p className="text-sm font-semibold text-red-800">Escalation Warning</p>
                        <p className="text-sm text-red-700">
                          This complaint is {daysOverdue > 0 ? `${daysOverdue} days overdue` : `${daysSinceCreated} days old`} and has not been resolved.
                          Please prioritize resolution or escalate to admin.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="mb-4 bg-gray-50 rounded-xl p-4">
                <div className="flex gap-2 mb-2">
                  ...
                  >
                    <Globe className="w-3 h-3" /> Public Remark
                  </button>
                </div>
                <textarea
                  value={noteContent}
                  onChange={e => setNoteContent(e.target.value)}
                  placeholder={noteType === 'internal' ? 'Internal note for staff/admin...' : 'Public remark visible to the submitter...'}
                  rows={3}
          ...
          <div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-20">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Change To</label>
                  <select
                    value={newStatus}
                  ...
                  >
                    ...
                    {allowedStatuses.map(s => (
                      <option key={s} value={s}>{s === 'in_progress' ? 'In Progress' : 'Resolved'}</option>
                    ))}
                  </select>
                </div>
